...
2. **Keep the MkDocs configuration collocated with the content (`docs/mkdocs.yml`).** Pass the path to MkDocs commands (for example, `mkdocs serve -f docs/mkdocs.yml`) so local previews and CI builds stay consistent.
