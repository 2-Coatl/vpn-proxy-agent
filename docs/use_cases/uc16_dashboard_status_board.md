...
@startuml
start
:Clear the terminal and print header banners;
:Display uptime and system resource usage;
:List running Docker containers when Docker is installed;
:Show SSH connection counts and tunnel status;
:Print timestamp for the dashboard snapshot;
stop
@enduml
```
