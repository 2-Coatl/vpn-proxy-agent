...
```mermaid
flowchart TD
    A[Start watchdog_mcp] --> B{nc available?}
    B -- Yes --> C[Test TCP connectivity]
    B -- No --> D[Log warning]
    C --> E{Port reachable?}
    E -- No --> X[Exit with error]
    D --> F
    E -- Yes --> F[Proceed to process check]
    F --> G{pgrep available?}
    G -- Yes --> H[Locate MCP process]
    H --> I{Process found?}
    I -- No --> X
    G -- No --> J[Log warning]
    J --> K
    I -- Yes --> K[Proceed to health probe]
    K --> L{curl available?}
    L -- Yes --> M[Call health endpoint]
    M --> N{HTTP success?}
    N -- No --> X
    L -- No --> O[Log warning]
