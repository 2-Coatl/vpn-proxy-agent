...
```mermaid
flowchart TD
    A[Start backup_daily] --> B[Log start]
    B --> C[Create SSH config tarball]
    C --> D[Create user data tarball]
    D --> E{Docker available?}
    E -- Yes --> F[Export containers]
    E -- No --> G[Log skip]
    F --> H[Prune old *.tar.gz]
    G --> H
    H --> I[Prune old *.tar]
    I --> J[Log completion]
    J --> K[End]
