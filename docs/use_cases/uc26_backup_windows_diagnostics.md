...
@startuml
start
:Display Windows system metadata and PowerShell version;
:Inspect SSH installation and client configuration;
:Check SOCKS5 tunnel listener state;
:Report VS Code, Docker, and Git availability;
:Test direct connectivity and proxy reachability;
:Print closing banner summarizing diagnostics;
stop
@enduml
```
