...
```mermaid
flowchart TD
    A[Start restart_services] --> B[Log restart initiation]
    B --> C{docker compose available?}
    C -- Yes --> D[docker compose down]
    C -- No --> E[Log skip]
    D --> F[Check SSH tunnel process]
    E --> F
    F --> G{Tunnel running?}
    G -- Yes --> H[pkill ssh -D 1080]
    G -- No --> I[Log no tunnel]
    H --> J[Restart sshd]
    I --> J
    J --> K[Log completion]
    K --> L[End]
