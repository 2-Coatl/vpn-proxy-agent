...
@startuml
start
:Systemd or an operator calls `scripts/run_mcp.sh`;
:Wrapper sources environment defaults and optional overrides from `/etc/mcp/mcp.env`;
:Validates binary permissions and port availability;
:Executes the MCP binary in the foreground with log redirection;
stop
@enduml
```
