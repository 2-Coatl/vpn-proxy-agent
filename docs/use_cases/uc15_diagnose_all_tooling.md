...
```mermaid
flowchart TD
    A[Start diagnose_all] --> B[Print system information]
    B --> C[Collect resource metrics]
    C --> D[Inspect SSH and firewall]
    D --> E[Summarize Docker state]
    E --> F[Check tunnels and WireGuard]
    F --> G[Test external APIs]
    G --> H[Assess updates & logins]
    H --> I[Report backup inventory]
    I --> J[Print completion banner]
```
