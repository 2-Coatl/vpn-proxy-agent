...
@startuml
start
:Create or reuse system principals for the MCP service;
:Install command-line dependencies including ripgrep and jq;
:Configure language runtimes using mise based on toolchain configuration;
:Apply Git proxy settings for outbound repository access when defined;
:Prepare runtime directories, binary stub, environment file, and log rotation policy;
:Deploy and enable the systemd unit responsible for the MCP service;
stop
@enduml
```
