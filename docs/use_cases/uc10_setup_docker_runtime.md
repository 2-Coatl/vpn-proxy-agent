...
```mermaid
flowchart TD
    A[Start setup_docker] --> B{Docker already installed?}
    B -- Yes --> X[Log info and exit]
    B -- No --> C[Download get.docker.sh]
    C --> D[Run installer]
    D --> E[Add user to docker group]
    E --> F[Install compose plugin]
    F --> G[Write daemon.json]
    G --> H[Restart & enable docker]
    H --> I[Report success]
```
