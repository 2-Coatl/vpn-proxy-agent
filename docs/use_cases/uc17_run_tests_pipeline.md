...
@startuml
start
:Prepare artifacts directory for coverage outputs;
:Ensure required Python modules are installed or downgrade expectations when coverage is missing;
:Run pytest with coverage and generate reports;
:Execute shell regression suite;
