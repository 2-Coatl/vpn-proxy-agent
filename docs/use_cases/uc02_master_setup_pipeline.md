...
```mermaid
flowchart TD
    A[Start master_setup] --> B[Update package lists]
    B --> C{Docker installed?}
    C -- No --> D[Download and run get-docker.sh]
    C -- Yes --> E[Log existing Docker]
    D --> F[Add user to docker group]
    E --> G
    F --> G[Install Docker Compose]
    G --> H[Configure UFW]
    H --> I[Install Fail2Ban]
    I --> J[Create directory structure]
    J --> K[Generate backup script]
    K --> L[Configure cron]
    L --> M[Install monitoring tools]
    M --> N[Optionally install Netdata]
    N --> O[Summarize and exit]
```
