...
```mermaid
flowchart TD
    A[Start validate_build] --> B[Determine version & prefix]
    B --> C[Check binary exists]
    C --> D{Binary present?}
    D -- No --> X[Exit with error]
    D -- Yes --> E[Inspect --version output]
    E --> F{Version matches?}
    F -- No --> X
    F -- Yes --> G[Run hello world script]
    G --> H[Import core modules]
    H --> I[Log success]
```
