...
@startuml
start
:Validate requested Python version format;
:Install compilation dependencies via apt;
:Download the CPython source tarball for the target version;
:Extract the source into the build directory;
:Configure with optimizations, shared library, and RPATH settings;
:Compile and install using `make altinstall`;
stop
@enduml
```
