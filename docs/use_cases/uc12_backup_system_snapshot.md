...
@startuml
start
:Compute timestamp and target archive path;
:Invoke `create_tarball` to package project directories and SSH configuration;
:Prune prior system backups older than seven days;
:Log completion and archive name;
stop
@enduml
```
