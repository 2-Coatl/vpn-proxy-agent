...
@startuml
start
:Inspect active listeners for the tunnel port;
:When the tunnel is missing, log the event and relaunch SSH dynamic forwarding;
:Sleep for 60 seconds before repeating the check;
stop
@enduml
```
