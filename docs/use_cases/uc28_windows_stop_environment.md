...
@startuml
start
:Identify SSH processes providing SOCKS5 tunneling;
:Forcefully stop those processes;
:Remove proxy environment variables from the session;
:Notify the operator that the environment is stopped;
stop
@enduml
```
