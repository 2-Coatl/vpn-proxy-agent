...
@startuml
start
:Validate that the SSH client configuration exists;
:Detect whether a tunnel is already running on port 1080;
:Launch a background dynamic port forwarding session when no tunnel exists;
:Pause briefly and verify the listener is active;
:Provide a sample command to exercise the SOCKS5 proxy;
stop
@enduml
```
